...
- **gnina 重打分**：gnina 1.0 论文报告，使用 CNN scoring function 对输出 pose 重打分时，在 redocking / cross-docking 的 Top1 指标上优于 AutoDock Vina scoring；定义 binding pocket 时，Top1 从 58%→73%（redocking）和 27%→37%（cross-docking）。
...
The repository tracks the Python/Bash workflow and small example inputs only. Local installed binaries, downloaded archives, logs, OMX state, and docking run outputs are intentionally ignored.
